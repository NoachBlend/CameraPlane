...
def calc_intersection_2d(pt1, pt2, pt3, pt4):
    ...
    if not (pt1[0] - pt2[0]):
        vertical = 1                                        # first line is vertical
    if not (pt3[0] - pt4[0]):
        vertical = vertical + 2                             # second line is vertical

    if not vertical:  # no vertical lines
...
def create_mesh_data(level, vi1, vi2, vi3, vi4, vertices, faces):
    # vi = vertex index
    if level:
        # calculate vertices
        c_vertices = len(vertices)
        # point middle +0
        vertices.append(calc_intersection_2d(vertices[vi1], vertices[vi3], vertices[vi2], vertices[vi4]))
        h_point = ((vertices[c_vertices][0] - 1), vertices[c_vertices][1], 0)
        # point middle upper +1
        vertices.append(calc_mid_2_points(vertices[vi1], vertices[vi2]))
        # point middle left +2
        vertices.append(calc_intersection_2d(vertices[vi2], vertices[vi3], vertices[c_vertices], h_point))
        # point middle bottom +3
        vertices.append(calc_mid_2_points(vertices[vi3], vertices[vi4]))
        # point middle right +4
        vertices.append(calc_intersection_2d(vertices[vi4], vertices[vi1], vertices[c_vertices], h_point))

        # Q1
        create_mesh_data(level - 1, vi1, c_vertices + 1, c_vertices, c_vertices + 4, vertices, faces)
        # Q2
        create_mesh_data(level - 1, c_vertices + 1, vi2, c_vertices + 2, c_vertices, vertices, faces)
        # Q3
        create_mesh_data(level - 1, c_vertices, c_vertices + 2, vi3, c_vertices + 3, vertices, faces)
        # Q4
        create_mesh_data(level - 1, c_vertices + 4, c_vertices, c_vertices + 3, vi4, vertices, faces)
    else:
...
def add_camera_plane(self, context):
    ...
    verts = [(cos(theta) * (end / sin(theta)), end, 0), (-cos(theta) * (end / sin(theta)), end, 0),
             (-cos(theta) * (start / sin(theta)), start, 0), (cos(theta) * (start / sin(theta)), start, 0)]
